	...
	return new Promise((resolve, reject) => {
		req.getConnection(function (error, database) {
			if (error) {
				...
				var query = 'SELECT c.idCompra,c.montoTotal,c.fechaRegistro,p.nombreProveedor, u.nombreUsuario, pro.nombreProducto, cp.cantidadProducto  FROM Compras c INNER JOIN Proveedores p  ON c.idProveedor=p.idProveedor INNER  JOIN Usuarios u  ON c.idUsuario=u.idUsuario INNER JOIN compras_productos cp ON c.idCompra= cp.idCompra INNER JOIN  Productos  pro ON cp.idProducto=pro.idProducto WHERE c.estado=1';
